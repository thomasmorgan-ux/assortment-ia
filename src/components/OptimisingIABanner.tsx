...
export function OptimisingIABanner({ onDismiss }: OptimisingIABannerProps) {
  return (
    <div
      className="flex w-full items-start gap-3 rounded-[6px] border border-[#a234da] p-4"
    ...
    >
      <div className="flex shrink-0 items-center justify-center">
        <div className="flex-none rotate-180 scale-y-[-1]">
          <Sparkles size={24} className="text-[#a234da]" aria-hidden />
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={onDismiss}
        className="flex h-10 shrink-0 items-center justify-center rounded-[4px] px-3 text-[#00050a] transition-colors hover:bg-[#a234da]/10"
        aria-label="Dismiss"
      >
        <X size={16} />
      </button>
